...
class MALATS(LMCTS):
    ...
    Only two hooks are overridden:
       _sample_theta  - draws one theta from the current posterior with MALA
       name           - pretty string for wandb / logging
    All hyper-parameters are copied from LMCTS, plus:
        mala_step_size   : gamma   (float)
    ...
    def _sample_theta(self, init_theta: torch.Tensor):
        ...
        return mala_sampling(
            init_theta.detach(),  # warm-start (previous theta or SGD estimate)
            logp_fn = logp_fn,
